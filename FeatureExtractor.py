...
class FE:
    def __init__(self,file_path,limit=np.inf):
        ...
        ### Prep pcap --- find the place of file and corresponding software(i.e., tshark) before extracting features ##  
        self.__prep__()
    ...
    def __prep__(self):
        ...
        ##If file is pcap
            ...
            if os.path.isfile(self._tshark):
                ...
                self.path += ".tsv"       # e.g., file path of mirai = mirai.pcap.tsv 
                self.parse_type = "tsv"      # tsv is parsed version of the pcap
        ...
        if self.parse_type == "tsv":
            maxInt = sys.maxsize   # sys.maxsize means how many rows the system can handle. so laptop and desktop present the # of packets.
            decrement = True
            while decrement:
            ...
            self.tsvinf = open(self.path, 'rt', encoding="utf8")  # read file line by line. file is about packet's header info and content
            self.tsvin = csv.reader(self.tsvinf, delimiter='\t')   # extract important info from header, end with \t
    ...
    def pcap2tsv_with_tshark(self):
        ...
        # define what attributes will be extracted. 19 key attributes have been extracted
    ...
    def get_num_features(self):
        return len(self.nstat.getNetStatHeaders())   # return a vector about packet feature.
